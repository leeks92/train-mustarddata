...
import { getSrtRoute, getSrtRoutes, getStations, formatCharge, getValidMinCharge, getValidMaxCharge, getMetadata } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${depDisplay}에서 ${arrDisplay} 가는 SRT 시간표 - 요금, 소요시간`,
    description: `${depDisplay}에서 ${arrDisplay} 가는 SRT 시간표. ${route.schedules.length}회 운행${timeText}${chargeText}.`,
    keywords: [
      `${depName} ${arrName} SRT`,
      `${depName} ${arrName} 기차`,
      `${depDisplay} SRT 시간표`,
...
export default async function SRTRoutePage({ params }: Props) {
  ...
  const faqItems = [
    {
      question: `${depName}에서 ${arrName}까지 SRT 요금은 얼마인가요?`,
      answer: `${depName}에서 ${arrName}까지 SRT 요금은 ${formatCharge(minCharge)}${minCharge !== maxCharge ? `부터 ${formatCharge(maxCharge)}` : ''}입니다. 열차 유형(${trainTypeList})에 따라 요금이 다릅니다.`,
    },
    {
      question: `${depName}에서 ${arrName}까지 SRT 첫차와 막차 시간은?`,
      answer: `첫차는 ${schedules[0]?.depTime || '-'}에 출발하고, 막차는 ${schedules[schedules.length - 1]?.depTime || '-'}에 출발합니다. 하루 총 ${schedules.length}회 운행됩니다.`,
    },
    {
      question: `${depName}에서 ${arrName}까지 SRT 소요시간은 얼마나 걸리나요?`,
      answer: `${depName}에서 ${arrName}까지 SRT 소요시간은 ${estimatedDuration || '노선에 따라 상이합니다'}입니다. 정차역에 따라 달라질 수 있습니다.`,
    },
    {
      question: `${depName} ${arrName} SRT 하루 몇 회 운행하나요?`,
      answer: `${depName}에서 ${arrName}까지 하루 총 ${schedules.length}회 운행됩니다. ${trainTypeList} 열차가 운행됩니다.`,
    },
    {
      question: `${depName} ${arrName} SRT 예매는 어디서 하나요?`,
      answer: `SRT 공식 사이트(etk.srail.kr) 또는 SR 앱에서 온라인 예매가 가능합니다. 출발 20분 전까지 무료 취소가 가능합니다.`,
    },
    {
      question: `${depName} ${arrName} SRT 어린이·청소년 할인이 되나요?`,
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <TableJsonLd
        name={`${depName} → ${arrName} SRT 시간표`}
        description={`${depName}에서 ${arrName}까지 SRT 시간표. ${schedules.length}회 운행, ${formatCharge(minCharge)}부터.`}
      ...
      <nav className="text-sm text-gray-600 mb-6">
        ...
        <span className="mx-2">›</span>
        <Link href={`/SRT/schedule/${depStationSlug}`} className="hover:text-purple-600">{depName}</Link>
        <span className="mx-2">›</span>
      ...
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{depName} → {arrName} SRT 요금 안내</h2>
      ...
      {schedules.length >= 3 && (
        <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-xl font-bold mb-4 text-gray-900">{depName} → {arrName} SRT 시간대별 운행</h2>
      ...
      {/* FAQ 섹션 */}
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{depName} → {arrName} SRT 자주 묻는 질문</h2>
      ...
      <section className="mt-12 bg-gray-100 rounded-lg p-6 text-sm text-gray-700 leading-relaxed">
        <h2 className="font-bold text-gray-900 mb-3">{depName} → {arrName} SRT 안내</h2>
        <div className="space-y-2">
          <p>
            {depName}에서 {arrName}까지 SRT는 하루 총 <strong>{schedules.length}회</strong> 운행됩니다.
...
function TransportCompareSection({ depName, arrName, minCharge, duration }: { depName: string; arrName: string; minCharge: number; duration: string | null }) {
  ...
  return (
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      <h2 className="text-xl font-bold mb-4 text-gray-900">
        <span className="mr-2" aria-hidden="true">&#x1F504;</span> SRT vs KTX 비교
      </h2>
