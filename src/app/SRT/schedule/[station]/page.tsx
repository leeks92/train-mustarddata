...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name} SRT 시간표 운행노선 요금 역 정보`,
    description: `${name}에서 출발하는 SRT 시간표. ${stationSrtRoutes.length}개 운행노선, 요금 정보.${stationInfoText} 전국 주요 도시 연결.`,
    keywords: [
      `${name} SRT`,
      `${name} SRT 시간표`,
      `${name} SRT 요금`,
    ...
    openGraph: {
      title: `${name} SRT 시간표 - 운행노선, 요금, 역 정보`,
      description: `${name}에서 출발하는 SRT 시간표. ${stationSrtRoutes.length}개 운행노선, 요금 정보.${stationInfoText}`,
      url: `${BASE_URL}/SRT/schedule/${decodedSlug}/`,
      type: 'website',
    },
    twitter: {
      card: 'summary',
      title: `${name} SRT 시간표 - 운행노선, 요금`,
      description: `${name}에서 출발하는 SRT 시간표. ${stationSrtRoutes.length}개 운행노선, 요금 정보.`,
    },
  };
}

export default async function SRTStationPage({ params }: Props) {
  const { station: stationSlug } = await params;
  const decodedSlug = decodeURIComponent(stationSlug);
  const stationId = getStationIdBySlug(decodedSlug);
  const station = stationId ? getStation(stationId) : null;
  ...
  // SRT 전용 데이터에서 해당 역 출발 노선만 필터
  const srtRoutes = srtAllRoutes.filter(r => r.depStationId === stationId);

  const sortedRoutes = srtRoutes.sort((a, b) =>
  ...
  const breadcrumbItems = [
    ...
    { name: name, url: `${BASE_URL}/SRT/schedule/${decodedSlug}/` },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
              ...
              {stationInfo.phone && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`tel:${stationInfo.phone}`} className="text-purple-600 hover:underline font-medium">
                      {stationInfo.phone}
                    </a>
                  </div>
                </div>
      ...
      <section>
        <h2 className="text-xl font-bold mb-4">{name} 출발 SRT 운행노선</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="text-sm text-gray-700 space-y-2">
              <li>
                <a href="https://etk.srail.kr" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">
                  SRT 예매 (etk.srail.kr) →
                </a>
              </li>
              <li>
                <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">
                  코레일 (Let&apos;s Korail) →
                </a>
              </li>
            </ul>
      ...
      <section className="mt-8 text-sm text-gray-600">
        <p>
          {name}에서 출발하는 SRT 시간표입니다.
          총 {sortedRoutes.length}개 노선이 운행되며, 전국 주요 도시로 연결됩니다.
          정확한 시간과 요금은 SRT 예매 사이트에서 확인하세요.
        </p>
      </section>
    </div>
  );
