...
export default function SRTListClient({ stations, routes }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-16">
      <div className="bg-purple-700 text-white py-12 px-4 shadow-md">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">SRT 시간표</h1>
          <p className="text-purple-200 text-lg mb-8">
            전국 <strong className="text-white">{activeStations.length}</strong>개 역, <strong className="text-white">{routes.length}</strong>개 노선의 SRT 운행 정보를 확인하세요
          </p>

          <div className="max-w-xl mx-auto relative">
            <input
              type="text"
              placeholder="역 이름 검색 (예: 수서, 부산)"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full py-4 px-6 rounded-full bg-white text-gray-900 shadow-lg focus:outline-none focus:ring-4 focus:ring-purple-400 text-lg placeholder-gray-400"
            />
            ...
            return (
              <section key={region} className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100">
                <h2 className="text-2xl font-bold mb-6 flex items-center gap-3 text-gray-800 border-b pb-4">
                  <span className="flex items-center justify-center w-10 h-10 rounded-full bg-purple-50 text-purple-600 text-lg">
                    ...
                    .map(station => {
                      ...
                      return (
                        <Link
                          ...
                          href={`/SRT/schedule/${stationSlug}`}
                          className="group block rounded-xl p-5 transition-all duration-200 bg-gray-50 hover:bg-white border border-transparent hover:border-purple-200 hover:shadow-md"
                        >
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="text-lg font-bold transition-colors text-gray-900 group-hover:text-purple-600">
                              {station.stationName}
                            </h3>
