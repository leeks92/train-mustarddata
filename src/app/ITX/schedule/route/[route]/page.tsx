...
import { getItxRoute, getItxRoutes, getStations, formatCharge, getValidMinCharge, getValidMaxCharge, getMetadata } from '@/lib/data';
...
function getTrainTypeBadge(trainType: string) {
  ...
  if (trainType.includes('ITX-새마을')) return 'bg-sky-100 text-sky-800';
  if (trainType.includes('ITX-마음')) return 'bg-sky-100 text-sky-800';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const route = getItxRoute(depStationId, arrStationId);
  ...
  return {
    title: `${route.depStationName}에서 ${route.arrStationName} 가는 ITX 시간표 - 요금, 소요시간`,
    description: `${route.depStationName}에서 ${route.arrStationName} 가는 ITX 시간표. ${route.schedules.length}회 운행${timeText}${chargeText}.`,
    keywords: [
      `${depName} ${arrName} ITX`,
      `${depName} ${arrName} ITX-새마을`,
      `${depName} ${arrName} ITX-청춘`,
    ...
    openGraph: {
      title: `${route.depStationName}에서 ${route.arrStationName} 가는 ITX 시간표`,
      description: `${route.depStationName}에서 ${route.arrStationName} 가는 ITX. ${route.schedules.length}회/일 운행${timeText}${chargeText}.`,
...
export default async function ITXRoutePage({ params }: Props) {
  const routeParam = (await params).route;
  const parsed = parseRouteSlug(routeParam);
  ...
  const breadcrumbItems = [
    ...
    { name: route.depStationName, url: `${BASE_URL}/ITX/schedule/${depStationSlug}` },
    { name: `${route.depStationName} → ${route.arrStationName}`, url: `${BASE_URL}/ITX/schedule/route/${routeSlug}` },
  ];
  ...
  const trainTypeList = [...new Set(schedules.map(s => s.trainType || 'ITX'))].join(', ');
  ...
  const faqItems = [
    {
      question: `${route.depStationName}에서 ${route.arrStationName}까지 ITX 요금은 얼마인가요?`,
      answer: `${route.depStationName}에서 ${route.arrStationName}까지 ITX 요금은 ${formatCharge(minCharge)}${minCharge !== maxCharge ? `부터 ${formatCharge(maxCharge)}` : ''}입니다. 열차 유형(${trainTypeList})에 따라 요금이 다릅니다.`,
    },
    {
    ...
    },
    {
      ...
      answer: `${route.depStationName}에서 ${route.arrStationName}까지 ITX 소요시간은 ${estimatedDuration || '노선에 따라 상이합니다'}입니다. 정차역에 따라 달라질 수 있습니다.`,
    },
    {
      question: `${route.depStationName} ${route.arrStationName} ITX 하루 몇 회 운행하나요?`,
      answer: `${route.depStationName}에서 ${route.arrStationName}까지 하루 총 ${schedules.length}회 운행됩니다. ${trainTypeList} 열차가 운행됩니다.`,
    },
    {
      question: `${route.depStationName} ${route.arrStationName} ITX 예매는 어디서 하나요?`,
      answer: `코레일(www.letskorail.com) 또는 코레일톡 앱에서 온라인 예매가 가능합니다. 출발 20분 전까지 무료 취소가 가능합니다.`,
    },
    {
      question: `ITX-새마을과 ITX-청춘의 차이점은 무엇인가요?`,
      answer: `ITX-새마을은 경부선·호남선·전라선 등 전국 주요 간선 노선을 운행하며, ITX-청춘은 경춘선(청량리-춘천) 구간을 전문으로 운행합니다. ITX-청춘은 자유석(입석) 이용이 가능하고, ITX-새마을은 전 좌석 지정석입니다.`,
    },
    {
      question: `${route.depStationName} ${route.arrStationName} ITX 어린이·청소년 할인이 되나요?`,
      answer: `만 6세~12세 어린이는 약 50% 할인, 만 13세~18세 청소년은 약 30% 할인이 적용됩니다. 예매 시 생년월일 입력이 필요합니다.`,
    },
  ];

  // 관련 노선
  const allItxRoutes = getItxRoutes();
  const sameDepRoutes = allItxRoutes
    .filter(r => r.depStationName === route.depStationName && r.arrStationName !== route.arrStationName)
    .slice(0, 4);
  const sameArrRoutes = allItxRoutes
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <TableJsonLd
        name={`${route.depStationName} → ${route.arrStationName} ITX 시간표`}
        description={`${route.depStationName}에서 ${route.arrStationName}까지 ITX 시간표. ${schedules.length}회 운행, ${formatCharge(minCharge)}부터.`}
      ...
      <nav className="text-sm text-gray-600 mb-6">
        ...
        <span className="mx-2">›</span>
        <Link href={`/ITX/schedule/${depStationSlug}`} className="hover:text-sky-600">{route.depStationName}</Link>
        <span className="mx-2">›</span>
      ...
      <header className="bg-gradient-to-r from-sky-600 to-sky-700 text-white rounded-xl p-6 mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-bold mb-4">
          {route.depStationName}에서 {route.arrStationName} 가는 ITX 시간표
        </h1>
      ...
      <div className="bg-sky-50 border border-sky-200 rounded-lg p-4 mb-8">
        <p className="text-sm text-sky-800">
          <strong>예매 안내:</strong> 정확한 좌석 확인과 예매는 코레일 공식 사이트를 이용해 주세요.
        </p>
        <div className="flex gap-4 mt-3">
          <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-sky-700 hover:underline text-sm font-medium">
            코레일 예매 →
          </a>
        </div>
      </div>
      ...
      <section className="bg-white rounded-xl shadow overflow-hidden">
        <div className="p-4 border-b bg-gray-50">
          <h2 className="text-lg font-bold text-gray-900">{route.depStationName} → {route.arrStationName} ITX 전체 시간표</h2>
        </div>
      ...
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{route.depStationName} → {route.arrStationName} ITX 요금 안내</h2>
      ...
      {schedules.length >= 3 && (
        <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-xl font-bold mb-4 text-gray-900">{route.depStationName} → {route.arrStationName} ITX 시간대별 운행</h2>
      ...
      {/* FAQ 섹션 */}
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{route.depStationName} → {route.arrStationName} ITX 자주 묻는 질문</h2>
        <div className="space-y-4">
          {faqItems.map((item, idx) => (
            <details key={idx} className="bg-white border border-gray-200 rounded-lg group">
              <summary className="p-4 cursor-pointer font-medium text-gray-900 hover:text-sky-600 transition-colors list-none flex items-center justify-between">
                {item.question}
      ...
      <section className="mt-8 p-4 bg-gray-50 rounded-lg">
        ...
        <Link href={`/ITX/schedule/route/${reverseRouteSlug}`} className="text-sky-600 hover:underline">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {sameDepRoutes.map((r, idx) => (
                    <Link
                      key={idx}
                      href={`/ITX/schedule/route/${createRouteSlug(r.depStationName, r.arrStationName)}`}
                      className="block bg-white border border-gray-200 rounded-lg p-3 hover:border-sky-300 hover:shadow-sm transition"
                    >
      ...
      <section className="mt-12 bg-gray-100 rounded-lg p-6 text-sm text-gray-700 leading-relaxed">
        <h2 className="font-bold text-gray-900 mb-3">{route.depStationName} → {route.arrStationName} ITX 안내</h2>
        <div className="space-y-2">
          <p>
            {route.depStationName}에서 {route.arrStationName}까지 ITX는 하루 총 <strong>{schedules.length}회</strong> 운행됩니다.
          ...
          <p>
            예매는 <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline">코레일</a>에서
    ...
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      ...
      {guide.tips && guide.tips.length > 0 && (
        <div className="mt-4 bg-sky-50 rounded-lg p-4">
          <h3 className="font-bold text-sky-800 mb-2 text-sm">이용 팁</h3>
          <ul className="space-y-1">
            {guide.tips.map((tip, idx) => (
              <li key={idx} className="text-sm text-sky-700 flex gap-2">
...
function TransportCompareSection({ depName, arrName, minCharge, duration }: { depName: string; arrName: string; minCharge: number; duration: string | null }) {
  // 주요 노선에 대한 ITX vs KTX 비교 데이터
  const compareRoutes: Record<string, { ktxPrice: string; ktxTime: string }> = {
    '서울역-대전역': { ktxPrice: '약 23,700원', ktxTime: '약 50분' },
    '서울역-부산역': { ktxPrice: '약 59,800원', ktxTime: '약 2시간 30분' },
    '서울역-동대구역': { ktxPrice: '약 43,500원', ktxTime: '약 1시간 40분' },
    '용산역-광주송정역': { ktxPrice: '약 43,000원', ktxTime: '약 1시간 40분' },
    '서울역-울산역': { ktxPrice: '약 52,200원', ktxTime: '약 2시간 10분' },
  };

  // 출발역-도착역으로 키 생성
  const routeKey = `${depName}-${arrName}`;
  const compareData = compareRoutes[routeKey];
  ...
  return (
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      <h2 className="text-xl font-bold mb-4 text-gray-900">
        <span className="mr-2">&#x1F504;</span> ITX vs KTX 비교
      </h2>
      <p className="text-sm text-gray-600 mb-4">{depClean} → {arrClean} 구간의 열차 유형별 비교입니다.</p>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-gray-200">
              <th className="py-3 px-4 text-left font-bold text-gray-900">구분</th>
              <th className="py-3 px-4 text-center font-bold text-sky-700 bg-sky-50 rounded-t-lg">ITX</th>
          ...
          <tbody>
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 px-4 text-center bg-sky-50 font-bold text-sky-700">{formatCharge(minCharge)}~</td>
              <td className="py-3 px-4 text-center text-gray-700">{compareData.ktxPrice}</td>
            </tr>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">소요시간</td>
              <td className="py-3 px-4 text-center bg-sky-50 font-bold text-sky-700">{duration || '-'}</td>
              <td className="py-3 px-4 text-center text-gray-700">{compareData.ktxTime}</td>
            </tr>
            <tr>
              <td className="py-3 px-4 font-medium text-gray-700">장점</td>
              <td className="py-3 px-4 text-center bg-sky-50 text-gray-700 text-xs">저렴한 요금, 자유석 이용 가능</td>
              <td className="py-3 px-4 text-center text-gray-700 text-xs">빠른 이동, 정시 운행</td>
            </tr>
          </tbody>
        </table>
      </div>
      <p className="text-xs text-gray-500 mt-3">* ITX 요금은 일반석 정상 요금 기준이며, 할인 및 변동이 있을 수 있습니다. KTX 요금은 일반석 기준입니다.</p>
    </section>
  );
}
    ...
    <section className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-6">
      ...
      <div className="space-y-3 text-sm text-amber-800">
        <div className="flex gap-3">
          ...
          <p><strong>설날·추석 연휴:</strong> {depName} → {arrName} 노선은 명절 약 2주 전부터 예매가 시작됩니다. 코레일(www.letskorail.com)에서 사전 예매를 하지 않으면 좌석 확보가 어려울 수 있습니다.</p>
        </div>
        <div className="flex gap-3">
          ...
          <p><strong>자유석(입석) 안내:</strong> ITX-청춘은 좌석이 매진되어도 자유석(입석)으로 탑승할 수 있습니다. ITX-새마을은 전 좌석 지정석으로 매진 시 이용이 불가합니다.</p>
        </div>
        <div className="flex gap-3">
