...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const route = getKtxRoute(depStationId, arrStationId);
  ...
  return {
    title: `${depDisplay}에서 ${arrDisplay} 가는 KTX 시간표 - 요금, 소요시간`,
    description: `${depDisplay}에서 ${arrDisplay} 가는 KTX 시간표. ${route.schedules.length}회 운행${timeText}${chargeText}.`,
    keywords: [
      `${depName} ${arrName} KTX`,
      `${depName} ${arrName} 기차`,
      `${depDisplay} 시간표`,
    ...
    openGraph: {
      title: `${depDisplay}에서 ${arrDisplay} 가는 KTX 시간표`,
      description: `${depDisplay}에서 ${arrDisplay} 가는 KTX. ${route.schedules.length}회/일 운행${timeText}${chargeText}.`,
...
export default async function KTXRoutePage({ params }: Props) {
  const routeParam = (await params).route;
  const parsed = parseRouteSlug(routeParam);
  ...
  if (!parsed) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 text-center">
        ...
        <Link href="/KTX/schedule" className="text-emerald-600 hover:underline">
          KTX 역 목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  const breadcrumbItems = [
    ...
    { name: depName, url: `${BASE_URL}/KTX/schedule/${depStationSlug}/` },
    { name: `${depName} → ${arrName}`, url: `${BASE_URL}/KTX/schedule/route/${routeSlug}/` },
  ];
  ...
  const trainTypeList = [...new Set(schedules.map(s => s.trainType || 'KTX'))].join(', ');
  ...
  const faqItems = [
    {
      question: `${depName}에서 ${arrName}까지 KTX 요금은 얼마인가요?`,
      answer: `${depName}에서 ${arrName}까지 KTX 요금은 ${formatCharge(minCharge)}${minCharge !== maxCharge ? `부터 ${formatCharge(maxCharge)}` : ''}입니다. 열차 유형(${trainTypeList})에 따라 요금이 다릅니다.`,
    },
    {
    ...
    },
    {
      ...
      answer: `${depName}에서 ${arrName}까지 KTX 소요시간은 ${estimatedDuration || '노선에 따라 상이합니다'}입니다. 정차역에 따라 달라질 수 있습니다.`,
    },
    {
      question: `${depName} ${arrName} KTX 하루 몇 회 운행하나요?`,
      answer: `${depName}에서 ${arrName}까지 하루 총 ${schedules.length}회 운행됩니다. ${trainTypeList} 열차가 운행됩니다.`,
    },
    {
      question: `${depName} ${arrName} KTX 예매는 어디서 하나요?`,
      answer: `코레일(www.letskorail.com) 또는 코레일톡 앱에서 온라인 예매가 가능합니다. 출발 20분 전까지 무료 취소가 가능합니다.`,
    },
    {
      question: `${depName} ${arrName} KTX 어린이·청소년 할인이 되나요?`,
      answer: `만 6세~12세 어린이는 약 50% 할인, 만 13세~18세 청소년은 약 30% 할인이 적용됩니다. 예매 시 생년월일 입력이 필요합니다.`,
    },
  ];

  // 관련 노선
  const allKtxRoutes = getKtxRoutes();
  const sameDepRoutes = allKtxRoutes
    .filter(r => r.depStationName === route.depStationName && r.arrStationName !== route.arrStationName)
    .slice(0, 4);
  const sameArrRoutes = allKtxRoutes
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <TableJsonLd
        name={`${depName} → ${arrName} KTX 시간표`}
        description={`${depName}에서 ${arrName}까지 KTX 시간표. ${schedules.length}회 운행, ${formatCharge(minCharge)}부터.`}
      ...
      <nav className="text-sm text-gray-600 mb-6">
        ...
        <Link href="/KTX/schedule" className="hover:text-emerald-600">KTX 시간표</Link>
        <span className="mx-2">›</span>
        <Link href={`/KTX/schedule/${depStationSlug}`} className="hover:text-emerald-600">{depName}</Link>
        <span className="mx-2">›</span>
        <span className="text-gray-800">{arrName}</span>
      </nav>

      {/* 노선 정보 헤더 */}
      <header className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-xl p-6 mb-8">
        <div className="flex items-center gap-2 mb-2">
          <span className="bg-white/20 px-2 py-1 rounded text-sm">KTX</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-bold mb-4">
          {depName}에서 {arrName} 가는 KTX 시간표
        </h1>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-8">
        ...
        <div className="flex gap-4 mt-3">
          <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:underline text-sm font-medium">
            코레일 예매 →
          </a>
          <a href="https://etk.srail.kr" target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:underline text-sm font-medium">
            SRT 예매 →
          </a>
        </div>
      </div>
      ...
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{depName} → {arrName} KTX 요금 안내</h2>
      ...
      {schedules.length >= 3 && (
        <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-xl font-bold mb-4 text-gray-900">{depName} → {arrName} KTX 시간대별 운행</h2>
      ...
      {/* FAQ 섹션 */}
      <section className="mt-8">
        <h2 className="text-xl font-bold mb-4">{depName} → {arrName} KTX 자주 묻는 질문</h2>
        <div className="space-y-4">
          {faqItems.map((item, idx) => (
            <details key={idx} className="bg-white border border-gray-200 rounded-lg group">
              <summary className="p-4 cursor-pointer font-medium text-gray-900 hover:text-emerald-600 transition-colors list-none flex items-center justify-between">
                {item.question}
      ...
      <section className="mt-8 p-4 bg-gray-50 rounded-lg">
        ...
        <Link href={`/KTX/schedule/route/${reverseRouteSlug}`} className="text-emerald-600 hover:underline">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {sameDepRoutes.map((r, idx) => (
                    <Link
                      key={idx}
                      href={`/KTX/schedule/route/${createRouteSlug(r.depStationName, r.arrStationName)}`}
      ...
      <section className="mt-12 bg-gray-100 rounded-lg p-6 text-sm text-gray-700 leading-relaxed">
        <h2 className="font-bold text-gray-900 mb-3">{depName} → {arrName} KTX 안내</h2>
        <div className="space-y-2">
          <p>
            {depName}에서 {arrName}까지 KTX는 하루 총 <strong>{schedules.length}회</strong> 운행됩니다.
          ...
          <p>
            예매는 <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline">코레일</a> 또는
            <a href="https://etk.srail.kr" target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline ml-1">SRT</a>에서
    ...
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      ...
      {guide.tips && guide.tips.length > 0 && (
        <div className="mt-4 bg-emerald-50 rounded-lg p-4">
          <h3 className="font-bold text-emerald-800 mb-2 text-sm">이용 팁</h3>
          <ul className="space-y-1">
            {guide.tips.map((tip, idx) => (
              <li key={idx} className="text-sm text-emerald-700 flex gap-2">
...
function TransportCompareSection({ depName, arrName, minCharge, duration }: { depName: string; arrName: string; minCharge: number; duration: string | null }) {
  // 주요 노선에 대한 고속버스 비교 데이터
  const busRoutes: Record<string, { busPrice: string; busTime: string; busTerminal: string }> = {
    '서울역-부산역': { busPrice: '약 23,000원', busTime: '약 4시간 20분', busTerminal: '서울경부 → 부산' },
    '서울역-동대구역': { busPrice: '약 19,000원', busTime: '약 3시간 30분', busTerminal: '서울경부 → 동대구' },
    '서울역-대전역': { busPrice: '약 10,000원', busTime: '약 2시간', busTerminal: '서울경부 → 대전복합' },
    '용산역-광주송정역': { busPrice: '약 23,000원', busTime: '약 3시간 30분', busTerminal: '센트럴시티 → 광주(유스퀘어)' },
    '청량리역-강릉역': { busPrice: '약 18,000원', busTime: '약 2시간 40분', busTerminal: '동서울 → 강릉' },
    '서울역-울산역': { busPrice: '약 32,000원', busTime: '약 4시간', busTerminal: '서울경부 → 울산' },
  };

  // 출발역-도착역으로 키 생성
  const routeKey = `${depName}-${arrName}`;
  const busData = busRoutes[routeKey];
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-gray-200">
              ...
              <th className="py-3 px-4 text-center font-bold text-emerald-700 bg-emerald-50 rounded-t-lg">KTX</th>
              <th className="py-3 px-4 text-center font-bold text-gray-700">고속버스</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">요금</td>
              <td className="py-3 px-4 text-center bg-emerald-50 font-bold text-emerald-700">{formatCharge(minCharge)}~</td>
              <td className="py-3 px-4 text-center text-gray-700">{busData.busPrice}</td>
            </tr>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">소요시간</td>
              <td className="py-3 px-4 text-center bg-emerald-50 font-bold text-emerald-700">{duration || '-'}</td>
              <td className="py-3 px-4 text-center text-gray-700">{busData.busTime}</td>
            </tr>
            <tr className="border-b border-gray-100">
              <td className="py-3 px-4 font-medium text-gray-700">탑승 위치</td>
              <td className="py-3 px-4 text-center bg-emerald-50 text-gray-700">{depName}</td>
              <td className="py-3 px-4 text-center text-gray-700">{busData.busTerminal}</td>
            </tr>
            <tr>
              <td className="py-3 px-4 font-medium text-gray-700">장점</td>
              <td className="py-3 px-4 text-center bg-emerald-50 text-gray-700 text-xs">빠른 이동, 정시 운행</td>
              <td className="py-3 px-4 text-center text-gray-700 text-xs">저렴한 요금, 좌석 선택 다양</td>
            </tr>
          </tbody>
        </table>
      </div>
      <p className="text-xs text-gray-500 mt-3">* KTX 요금은 일반석 정상 요금 기준이며, 할인 및 변동이 있을 수 있습니다. 고속버스 요금은 일반 기준입니다.</p>
    </section>
  );
}
    ...
    <section className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-6">
      ...
      <div className="space-y-3 text-sm text-amber-800">
        ...
        <div className="flex gap-3">
          ...
          <p><strong>임시 열차:</strong> 명절 연휴, 여름 휴가철(7~8월), 겨울 스키시즌에는 임시 열차가 추가 배차됩니다. 정확한 시간표는 출발일 기준 코레일 예매 사이트에서 확인하세요.</p>
        </div>
        <div className="flex gap-3">
