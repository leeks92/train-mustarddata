...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${name}에서 출발하는 KTX 시간표. ${stationKtxRoutes.length}개 운행노선, 요금 정보.${stationInfoText} 전국 주요 도시 연결.`,
    keywords: [
      `${name} KTX`,
      `${name} KTX 시간표`,
      `${name} 기차 요금`,
    ...
    twitter: {
      card: 'summary',
      title: `${name} KTX 시간표 - 운행노선, 요금`,
      description: `${name}에서 출발하는 KTX 시간표. ${stationKtxRoutes.length}개 운행노선, 요금 정보.`,
    },
  };
}
...
export default async function KTXStationPage({ params }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <section>
        <h2 className="text-xl font-bold mb-4">{name} 출발 KTX 운행노선</h2>
