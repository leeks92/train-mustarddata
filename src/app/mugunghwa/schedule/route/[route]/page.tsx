...
import { getMugunghwaRoute, getMugunghwaRoutes, getStations, formatCharge, getValidMinCharge, getValidMaxCharge } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const route = getMugunghwaRoute(depStationId, arrStationId);
  ...
  return {
    title: `${depName} → ${arrName} 무궁화호 시간표 - 요금, 소요시간`,
    description: `${route.depStationName}에서 ${route.arrStationName} 가는 무궁화호 시간표. ${route.schedules.length}회 운행${chargeText}.`,
    keywords: [
    ...
    openGraph: {
      title: `${depName} → ${arrName} 무궁화호 시간표`,
      description: `${route.depStationName}에서 ${route.arrStationName} 가는 무궁화호. ${route.schedules.length}회/일 운행${chargeText}.`,
...
export default async function MugunghwaRoutePage({ params }: Props) {
  const routeParam = (await params).route;
  const parsed = parseRouteSlug(routeParam);
  ...
  const trainTypeList = [...new Set(schedules.map(s => s.trainType || '무궁화호'))].join(', ');
  ...
  const faqItems = [
    ...
    },
    {
      ...
      answer: `무궁화호는 좌석이 모두 매진되어도 자유석(입석) 승차권을 구매하여 탑승할 수 있습니다. 자유석은 빈 좌석에 앉거나 통로에 서서 이동하는 방식으로, 좌석 요금 대비 약간 저렴합니다.`,
    },
    {
      question: `무궁화호와 누리로의 차이점은 무엇인가요?`,
  ...
  // 관련 노선
  const allMugunghwaRoutes = getMugunghwaRoutes();
  const sameDepRoutes = allMugunghwaRoutes
    .filter(r => r.depStationName === route.depStationName && r.arrStationName !== route.arrStationName)
    .slice(0, 4);
  const sameArrRoutes = allMugunghwaRoutes
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-8">
        <p className="text-sm text-orange-800">
          <strong>예매 안내:</strong> 정확한 좌석 확인과 예매는 공식 사이트를 이용해 주세요. 무궁화호는 자유석(입석) 이용이 가능합니다.
        </p>
        <div className="flex gap-4 mt-3">
          <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-orange-700 hover:underline text-sm font-medium">
            코레일 예매 →
          </a>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {faqItems.map((item, idx) => (
            <details key={idx} className="bg-white border border-gray-200 rounded-lg group">
              <summary className="p-4 cursor-pointer font-medium text-gray-900 hover:text-orange-600 transition-colors list-none flex items-center justify-between">
                {item.question}
      ...
      <section className="mt-12 bg-gray-100 rounded-lg p-6 text-sm text-gray-700 leading-relaxed">
        ...
        <div className="space-y-2">
          ...
          <p>
            무궁화호는 KTX에 비해 요금이 저렴하며, 자유석(입석) 이용이 가능하여 좌석 매진 시에도 탑승할 수 있습니다.
            예매는 <a href="https://www.letskorail.com" target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:underline">코레일</a>에서
            온라인으로 가능합니다.
          </p>
        </div>
      </section>
    ...
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      ...
      {guide.tips && guide.tips.length > 0 && (
        <div className="mt-4 bg-orange-50 rounded-lg p-4">
          <h3 className="font-bold text-orange-800 mb-2 text-sm">이용 팁</h3>
...
function TransportCompareSection({ depName, arrName, minCharge, duration }: { depName: string; arrName: string; minCharge: number; duration: string | null }) {
  // 주요 노선에 대한 무궁화호 vs KTX 비교 데이터
  const compareRoutes: Record<string, { mugunghwaPrice: string; mugunghwaTime: string; ktxPrice: string; ktxTime: string }> = {
    '서울역-대전역': { mugunghwaPrice: '10,900원', mugunghwaTime: '약 2시간 20분', ktxPrice: '23,700원', ktxTime: '약 50분' },
    '서울역-부산역': { mugunghwaPrice: '28,600원', mugunghwaTime: '약 5시간 30분', ktxPrice: '59,800원', ktxTime: '약 2시간 30분' },
    '서울역-경주역': { mugunghwaPrice: '28,600원', mugunghwaTime: '약 4시간 30분', ktxPrice: '43,500원 (신경주)', ktxTime: '약 2시간' },
  };
  ...
  return (
    <section className="mt-8 bg-white border border-gray-200 rounded-xl p-6">
      <h2 className="text-xl font-bold mb-4 text-gray-900">
        <span className="mr-2">&#x1F504;</span> 무궁화호 vs KTX 비교
      </h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b-2 border-gray-200">
              ...
              <th className="py-3 px-4 text-center font-bold text-orange-700 bg-orange-50 rounded-t-lg">무궁화호</th>
          ...
          <tbody>
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 px-4 text-center bg-orange-50 font-bold text-orange-700">{compareData.mugunghwaPrice}</td>
            ...
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 px-4 text-center bg-orange-50 text-gray-700">{compareData.mugunghwaTime}</td>
              <td className="py-3 px-4 text-center font-bold text-emerald-700">{compareData.ktxTime}</td>
            </tr>
            <tr>
              <td className="py-3 px-4 font-medium text-gray-700">장점</td>
              <td className="py-3 px-4 text-center bg-orange-50 text-gray-700 text-xs">저렴한 요금, 중간역 정차, 자유석 이용 가능</td>
              <td className="py-3 px-4 text-center text-gray-700 text-xs">빠른 이동</td>
            </tr>
          </tbody>
        </table>
      </div>
      <p className="text-xs text-gray-500 mt-3">* 요금은 일반석 정상 요금 기준이며, 할인 및 변동이 있을 수 있습니다.</p>
    </section>
  );
}
    ...
    <section className="mt-8 bg-amber-50 border border-amber-200 rounded-xl p-6">
      ...
      <div className="space-y-3 text-sm text-amber-800">
        <div className="flex gap-3">
          ...
          <p><strong>설날·추석 연휴:</strong> {depName} → {arrName} 노선은 명절 약 2주 전부터 예매가 시작됩니다. 코레일(www.letskorail.com)에서 사전 예매가 필요합니다.</p>
        </div>
        <div className="flex gap-3">
          ...
          <p><strong>자유석(입석) 안내:</strong> 무궁화호는 좌석이 매진되어도 자유석(입석) 승차권으로 탑승 가능합니다. 다만 명절 혼잡 시 자유석도 만석이 되어 탑승이 제한될 수 있습니다.</p>
        </div>
        <div className="flex gap-3">
